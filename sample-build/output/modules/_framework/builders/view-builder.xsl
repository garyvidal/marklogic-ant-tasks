       ...
       <h2><xsl:value-of select="($model/@label,$controller/@label,$controller/@name)[1]"/></h2>
         <div class="tools">
          <a class="ui-button ui-button-text ui-state-default ui-corner-all">
           ...
           <span class="ui-icon ui-icon-plusthick"></span>New</a>
          <a class="ui-button ui-button-text ui-state-default ui-corner-all">
           <xsl:attribute name="href">edit.html<xsl:value-of select="builder:get-primary-field-id($model)"/>=</xsl:attribute>
    ...
    <form name="form_{$model/@name}" method="get" action="/{$controller/@name}/edit.html"> 
     <ul>
      ...
      <li>
       ...
       <span>{response:body()/*:<xsl:value-of select="@name"/>}{" "}</span><br/>
      </li>
      </xsl:for-each>
      <li class="buttons">
...
<xsl:template name="html-new-template">
 ...
 <xsl:variable name="body">
  <div id="form-wrapper">
   ...
   <div class="content-box">
    <form name="form_{$model/@name}" method="post" action="/{$controller/@name}/save.html">
     <ul>
      <xsl:for-each select="$model//(domain:element|domain:attribute)">
       <xsl:variable name="type" select="./@type"></xsl:variable>
       <li>
        ...
        <div>
         <xsl:choose>
          ...
          <xsl:when test="$type = 'boolean'">
           <input class="field text small" name="{./@name}" type="checkbox" value="true" />  
          </xsl:when>
...
<xsl:template name="html-edit-template">
 ...
 <xsl:variable name="body">
  <div id="form-wrapper" xmlns="http://www.w3.org/1999/xhtml">
   <div class="inner-page-title">
    <h2>Edit <xsl:value-of select="($model/@label,$controller/@label,$controller/@name)[1]"/></h2>
   </div>
   <div class="content-box">
    <form name="form_{$model/@name}" method="post" action="/{$controller/@name}/save.html"> 
     <ul class="editPanel">
      ...
      <li class="buttons">
        ...
        <button id="cancelButton" class="ui-state-default ui-corner-all ui-button" type="button">Cancel</button>
      </li>
     </ul>
    </form>
   </div>
...
var gridModel = {{
     ...
     height: '500',
     multiselect: false,
     xmlReader : xmlListReaderSettings('<xsl:value-of select="builder:get-primary-field-id($model)"/>','<xsl:value-of select="$model/@name"/>s','<xsl:value-of select="$model/@name"/>'),
     onSelectRow: function(id){{ 
       if(id){{ 
         editForm('<xsl:value-of select="$controller/@name"/>','<xsl:value-of select="builder:get-primary-field-id($model)"/>',id,true);
       }} 
     }}
}};
...
<xsl:function name="builder:form-field">
  ...
  <xsl:choose>
   ...
   <xsl:when test="$datatype eq 'boolean'">
    <xsl:copy-of select="builder:text-field(generate-id($field),$field/@name,'field text','checkbox',$value)"/>
   </xsl:when>
   <xsl:otherwise>
