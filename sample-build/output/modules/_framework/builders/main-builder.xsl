...
<xsl:function name="builder:controller-module-uri">
 ...
 <xsl:function name="builder:resolve-as-type">
  ...
  <xsl:choose>
   <xsl:when test="starts-with($type, 'xs:')"> as <xsl:value-of select="$type"/></xsl:when>
 ...
 <!--
  Resolves the Type for the given model element/attribute
 -->
 <xsl:function name="builder:cast-as-type">
  <xsl:param name="model-node"/>
  <xsl:variable name="type" select="builder:resolve-datatype($model-node/@type)"/>
  <xsl:choose>
   <xsl:when test="starts-with($type, 'xs:')"> cast as <xsl:value-of select="$type"/></xsl:when>
   <xsl:when test="$type = 'schema-element'"/>
   <xsl:otherwise> cast as <xsl:value-of select="local-name($model-node)"/>(<xsl:value-of select="tokenize($model-node/@type,':')[2]"/>)</xsl:otherwise>   
  </xsl:choose>
 </xsl:function> 
