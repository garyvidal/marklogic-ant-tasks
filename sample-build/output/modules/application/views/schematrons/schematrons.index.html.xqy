...
<div xmlns="http://www.w3.org/1999/xhtml" id="form-wrapper">
   ...
   <div class="list-box">
      <table id="schematron_table"/>
      <div id="schematron_table_pager"/>
   </div>
...
var gridModel = {{
     url: '/schematrons/list.xml',
     pager: jQuery('#schematron_table_pager'),
     datatype: "xml",
     colModel:[
  {{ name:'id', label:'Schematron ID', index:'id', xmlmap:'id', 
  ...
  {{ name:'schematronElement', label:'Schematron Element', index:'schematronElement', xmlmap:'schematronElement', 
     jsonmap:'schematronElement', dataType:'', resizable: true, sortable: true,  width:'80'}}],
     loadonce:true,
     rowNum:9999999,
     pgbuttons: false,
    loadonce:true,
    ...
    pgbuttons: false,
     ...
     //Grid Text
     emptyrecords: "No schematron's Found",
     loadtext: "Loading schematron's",
     gridview: true,
     viewrecords :true,
     rownumbers:true,
     width: '500',
     height: '500',
     multiselect: false,
     xmlReader : xmlListReaderSettings('id','schematrons','schematron'),
     onSelectRow: function(id){{ 
       if(id){{ 
         editForm('schematrons','id',id,true);
       }} 
     }}
}};
...
$(document).ready(function(){{
   initListGrid("#schematron_table",gridModel)
}});
</script>
</div>