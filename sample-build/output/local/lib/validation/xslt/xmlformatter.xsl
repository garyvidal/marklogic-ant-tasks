...
<xsl:stylesheet version="1.0"
   ...
   <xsl:template name="format-message">
    ...
    <xsl:variable name="message-elem" select="xdmp:value(concat('$root',$message/*:location))"/>
   	<xsl:variable name="type">
   		<xsl:choose>
   		  <xsl:when test="local-name($message) = 'error'">Error</xsl:when>
   		  <xsl:when test="local-name($message) = 'warning'">Alert</xsl:when>
   		  <xsl:when test="local-name($message) = 'info'">Info</xsl:when>
   		  <xsl:otherwise/>
   		</xsl:choose>
   	</xsl:variable>
   	<xsl:variable name="icon">
   		<xsl:choose>
   		  <xsl:when test="local-name($message) = 'error'">ui-icon-alert</xsl:when>
   		  <xsl:when test="local-name($message) = 'warning'">ui-icon-notice</xsl:when>
   		  <xsl:when test="local-name($message) = 'info'">ui-icon-info</xsl:when>
   		  <xsl:otherwise/>
   		</xsl:choose>
   	</xsl:variable>   	
   	<xsl:variable name="state">
   		<xsl:choose>
   		  <xsl:when test="local-name($message) = 'error'">ui-state-error</xsl:when>
   		  <xsl:when test="local-name($message) = 'warning'">ui-state-highlight</xsl:when>
   		  <xsl:when test="local-name($message) = 'info'">ui-state-default</xsl:when>
   		  <xsl:otherwise/>
   		</xsl:choose>
   	</xsl:variable>  
	 	<div class="ui-widget">
			<div style="padding: 0.7em;" class="{$state} ui-corner-all"> 
				<p><a href="#{generate-id($message-elem)}"><span style="float: left; margin-right: .3em;" class="ui-icon {$icon}">&nbsp;</span> 
				<strong><xsl:value-of select="$type"/>:&nbsp;</strong>
				<xsl:value-of select="$message/*:message"/>
				</a><br/>
				<strong>Path:</strong><xsl:value-of select="$message/*:location"/>
				</p>
			</div>
		</div>
   </xsl:template>
   ...
   <xsl:template match="*" mode="xmlverb">
      ...
      <xsl:variable name="location" select="xdmp:path(.)"/>
      <xsl:variable name="error" select="$errors/*[*:location eq $location]"/>
      <xsl:variable name="etype">
		<xsl:if test="exists($error)">
				<xsl:choose>
				  <xsl:when test="local-name($error) = 'error'">ERROR:</xsl:when>
				  <xsl:when test="local-name($error) = 'warning'">WARNING:</xsl:when>
				  <xsl:when test="local-name($error) = 'info'">INFO:</xsl:when>
      ...
      <a id="{generate-id(.)}">
        <xsl:attribute name="class">xmlverb-element-wrapper  
		<xsl:if test="exists($error)">
			<xsl:choose>
			  <xsl:when test="local-name($error) = 'error'">xml_error</xsl:when>
			  <xsl:when test="local-name($error) = 'warning'">xml_warn</xsl:when>
			  <xsl:when test="local-name($error) = 'info'">xml_info</xsl:when>
			  <xsl:otherwise></xsl:otherwise>
			</xsl:choose>
		</xsl:if>
      </xsl:attribute>
      <xsl:if test="exists($error)">
      	<xsl:attribute name="title"><xsl:value-of select="$etype"/><xsl:value-of select="normalize-space($error/*:message)"/></xsl:attribute>
      </xsl:if>
      <xsl:if test="$indent-elements">
