...
{
   model:get(request:param("<xsl:value-of select="$identity/@name"/>"))
};
 </xsl:function>
...
declare function controller:search()
{
 ...
 <xsl:function name="builder:controller-update-params">
  ...
  <xsl:choose>
   ...
   request:param("<xsl:value-of select="$id-elem/@name"/>") <xsl:value-of select="builder:cast-as-type($id-elem)"/>,</xsl:when>
  ...
  <xsl:for-each select="$model//(domain:element|domain:attribute)[not(@identity = 'true')]"> 
   request:param("<xsl:value-of select="./@name"/>") <xsl:value-of select="builder:cast-as-type(.)"/><xsl:if test="position() ne last()">,</xsl:if></xsl:for-each>
 </xsl:function> 
 ...
 <xsl:function name="builder:controller-create-params">
  <xsl:param name="model" as="element(domain:model)"/>
  <xsl:for-each select="$model//(domain:element|domain:attribute)[not(@identity = 'true')]"> 
  request:param("<xsl:value-of select="./@name"/>") <xsl:value-of select="builder:resolve-datatype(.)"/><xsl:if test="position() ne last()">,</xsl:if></xsl:for-each>
 </xsl:function> 
